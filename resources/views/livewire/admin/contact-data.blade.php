                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="container py-3">
                        <template  x-for="contact in contacts" :key="contact.id">
                            <div class="card-body py-5 m-4"  style="background-color: #efeff0">
                                ...
                                <div x-data="{reply: false, feedbackReply: @entangle('feedbackReply')}" class="col-md-4 mt-6" >
                                    <div>
                                        <a @click.prevent = "reply = true" href="#">Reply</a>
                                    </div>
                                    <div x-show="reply" @click.outside="reply = false">
                                            <textarea cols="30" rows="5" class="form-control @error('feedbackReply') is-invalid @enderror" x-model="feedbackReply" placeholder="Your Reply... "></textarea>
                                            @error('feedbackReply')<span class="invalid-feedback" role="alert" style="color:red"><strong>{{ $message }}</strong></span>@enderror
                                            <button  x-on:click ="$wire.feedback(contact.user_id,contact.id)" class="btn btn-primary mt-2">Send</button>
                                    </div>
                                </div>
                            </div>
                        </template>
